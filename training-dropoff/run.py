...
def main(data_path: str = DATA_PATH, model_path: str = DROPOFF_MODEL_PATH):
    ...
    model = train_dropoff_model(train, val, model_path)

    print("Evaluating model...")
    metrics = evaluate(model, val, FEATURE_COLUMNS, LABEL_COLUMN)

    print("\n" + "=" * 40)
    print("DROPOFF ETA MODEL METRICS")
    print("=" * 40)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train dropoff ETA prediction model")
    parser.add_argument(
        "--data", 
        default=DATA_PATH, 
        help="Path to dropoff trips parquet file"
    )
    parser.add_argument(
        "--output", 
        default=DROPOFF_MODEL_PATH, 
